...
let artistName = MOCK_DATA;

for( let index in artistName) {
  describe('Test para crear un artista', () => {
    ...
    it('Types the name of the artist', () => {
      cy.get('input[id="name"]').type(artistName[index].title);
      cy.wait(1000);
    })
    ...
    it('Checks that the new artist is included in the list', () => {
      cy.get('dd[class="artistsNames"]').contains(artistName[index].title).should('exist');
    })
  })
}
