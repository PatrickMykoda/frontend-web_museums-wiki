...
import MOCK_DATA from './schemas_mockaroo/museumCreateNegative.json'

let museumName = MOCK_DATA;

for( let index in museumName) {
describe('Test para crear un museo', () => {
  ...
  it('Types the name of the museum', () => {
    cy.get('input[id="name"]').type(museumName[index].title);
    cy.wait(1000);
  })
  ...
  it('Checks that the new museum is included in the list', () => {
    cy.get('dd[class="museumsNames"]').contains(museumName[index].title).should('exist');
  })
})
}
